...
public class Create_booking_Invalid_data_in_request extends FrameworkConstants {
...
//                       ********** Reading the xml request file **********
        ...
        ExtentLogger.logXMLResponse(response.asPrettyString());
            ...
            if(response.getBody().asString().contains("INVLD BRD PNT") ){
                flightFound = true;
            }
        ...
        BufferedWriter writer = new BufferedWriter(new FileWriter(getResponseDirectory() + "CreateBookingService\\Create_booking_Invalid_data_in_request.xml"));
        writer.write(response.asPrettyString());
        writer.close();

        Assert.assertTrue(response.getBody().asString().contains("Sell Itinerary Process Failed to Complete Successfully :  (1) INVLD BRD PNT"));
        ExtentLogger.info("Response contains \"Sell Itinerary Process Failed to Complete Successfully :  (1) INVLD BRD PNT\"");
...
//        ********** Reading Testdata from Excel ************
        ...
        XSSFRow InputRow = sheet.getRow(41);


        String filepath1;
        filepath1 = getRequestDirectory() + "CreateBookingService\\Create_booking_Invalid_data_in_request.xml";
...
//        XMLParser.updateAttributeValueatIndex("air1:FlightSegment", "FlightNumber", InputRow.getCell(6).getStringCellValue(), getTemp_requestPath(),1);
        XMLParser.updateAttributeValueatIndex("air1:FlightSegment", "FlightNumber", availableFlights.get(InputRow.getCell(3).getStringCellValue() + "-" + InputRow.getCell(4).getStringCellValue()).get(i), getTemp_requestPath(),1);
