...
//    ******** Read the updated request and send it to fetch the response *********
        ...
        ExtentLogger.logXMLResponse(response.asPrettyString());
            if(response.getBody().asString().contains("Success") && response.getBody().asString().contains("Status=\"Open\"")){
                flightFound = true;
            }
...
//        XMLParser.SetTagtextatIndex("read:FlightNumber", InputRow.getCell(1).getStringCellValue(), filepath1, 0);
        //XMLParser.SetTagtextatIndex("read:FlightNumber",flightNumber, filepath1, 0);
        ...
        XMLParser.updateAttributeValue("read:DepartureAirport", "CodeContext", InputRow.getCell(4).getStringCellValue(), getTemp_requestPath());
